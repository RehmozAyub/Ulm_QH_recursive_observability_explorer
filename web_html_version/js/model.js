...
(function (global) {
  ...
  function rhs(tau, y, p, dynamic) {
    ...
    // dI/dtau = a*A*I + b*A_rec*F(I) - [c*R*I] - sI*I^2
    // Damping term gated by term_damping (reduced stage models, manuscript 7.1-7.3).
    ...
    // Building acts on remaining headroom, erosion is proportional to R,
    // so R is confined to [0, 1] structurally.
    ...
    // dO/dtau = production - (O - O_floor) * suppression
    // Compression/stealth are fractional suppression rates, so O >= O_floor.
    const production =
    ...
    const O_floor = (p.O_floor === undefined) ? 0.0 : p.O_floor;
    const dO = production - (O - O_floor) * suppression;
    return [dI, dR, dO];
  }
  ...
  function rk45(fn, t0, t1, y0, rtol, atol, iAbort) {
    // Returns array of {t, y} for dense output; we sample later.
    // iAbort mirrors ModelParams.I_abort in the Python reference: superlinear
    // and exponential kernels blow up in finite tau, so stop cleanly instead
    // of grinding against the singularity.
    const I_ABORT = (iAbort === undefined || !(iAbort > 0)) ? 1e6 : iAbort;
    const n = y0.length;
    let t = t0;
    let y = y0.slice();
    let f = fn(t, y);
    ...
    while ((t1 - t) * dir > 0) {
      ...
      // Runaway guard: capability has diverged past the abort threshold.
      if (Math.abs(yNew[0]) > I_ABORT) { break; }
  ...
  function integrate(params, y0, tauSpan) {
    ...
    const sol = rk45(fn, tauSpan[0], tauSpan[1], y0vec, rtol, atol, p.I_abort);
